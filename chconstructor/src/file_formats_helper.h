	...
	struct SimpleReader
	{
		...
		static GraphInData<NodeT, EdgeT> readGraph(std::istream& is)
		{
			...
			for (EdgeID i = 0; i < nr_of_edges; ++i) {
				...
				else if (edge.id == constant::NO_EID) {
	...
	{       
		...
		static GraphInData<NodeT,EdgeT> readGraph(std::istream& is)
		{
			...
			Print("Read all the edges.");
                        ...
                        uint waycount = 0;
                        waycount = impl.readWayCount();
                        result.wayInfos.reserve(waycount);
                        Print("Number of ways: " << waycount);
                        
                        for (uint i = 0; i < waycount; ++i) {
                            WayInfo wayInfo = impl.readWay();
                            result.wayInfos.push_back(std::move(wayInfo));
                        }                        
                        Print("Read all the ways.");
                        
                        //Regenerate Directed Ways                       
                        EdgeID currentEdgeID(0);
                        EdgeID nextEdgeID;
                        WayID wayID(0);
                        
                        for (std::vector<WayInfo>::iterator it = result.wayInfos.begin(); it != result.wayInfos.end(); it++) {
                            if (it->undirected) {
                                //skip
                                currentEdgeID += it->waylength * 2;
                            } else {
                                
                                nextEdgeID = currentEdgeID + it->waylength;
                                DirectedWay<NodeT> directedWay;
                                assert(it->waylength != 0);
                                
                                //first node in way
                                NodeT &node = result.nodes[result.edges[currentEdgeID].src];
                                directedWay.nodes.emplace_back(node);
                                node.ways.emplace_back(directedWay);
                                
                                for (EdgeID i = currentEdgeID; i < nextEdgeID; i++) {
                                    NodeT &node = result.nodes[result.edges[currentEdgeID].tgt];
                                    directedWay.nodes.emplace_back(node);
                                    node.ways.emplace_back(directedWay);
                                }
                                ...
                                result.directedWays.push_back(directedWay);
                                currentEdgeID = nextEdgeID;
                                wayID++;
                            }
                        }
                        //assert(currentEdgeID == nr_of_edges);
