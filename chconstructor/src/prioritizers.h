...
{
        enum class State {start, removingDeadEnds, removingChains, removingRemaining, done};
	...
	public:
		DPPrioritizer(GraphT const& base_graph, CHConstructorT const& chc)
			: EdgeDiffPrioritizer<GraphT, CHConstructorT>(base_graph, chc), state(State::start),
                                ...
                                CaR(), priolists(), epsilon(0.01), roundcounter(1) { }		
                std::vector<NodeID> extractNextNodes();
...
std::vector<NodeID> DPPrioritizer<GraphT, CHConstructorT>::_chooseIndependentSetFromRemainder() {    
	...
	edge_diff_mean /= independent_set.size();
        ...
        //parameter to slow down remainder contraction compared to chain contraction
        double slowdown = 0.2;
	...
	for (size_t i(0); i<independent_set.size(); i++) {
		if (edge_diffs[i] <= edge_diff_mean) {
			...
			low_edge_diff_nodes.push_back(node);
                ...
                if (low_edge_diff_nodes.size() > slowdown*independent_set.size()) {
                    break;
                }                                
