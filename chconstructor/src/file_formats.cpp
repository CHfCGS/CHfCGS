...
namespace chc {
	...
	FileFormat toFileFormat(std::string const& format)
	{
		if (format == "STD") {
			...
			return FileFormat::FMI_DIST;
                ...
                else if (format == "FMI_DIST_WAY") {
			return FileFormat::FMI_DIST_WAY;
		}
			...
			return FileFormat::FMI_CH;
                ...
                else if (format == "FMI_CH_WAY") {
			return FileFormat::FMI_CH_WAY;
		}
	...
	void text_writeNode<OSMNode>(std::ostream& os, OSMNode const& node)
	{
        ...
        template<>
	void text_writeNode<WayOSMNode>(std::ostream& os, WayOSMNode const& node)
	{
		os << node.id << " " << node.osm_id << " " << node.lat << " "
			<< node.lon << " " << node.elev << "\n";
	}
	...
	void text_writeNode<CHNode<OSMNode>>(std::ostream& os, CHNode<OSMNode> const& node)
	{
        ...
        template<>
        void text_writeNode<CHNode<WayOSMNode>>(std::ostream& os, CHNode<WayOSMNode> const& node)
	{
		os << node.id << " " << node.osm_id << " " << node.lat << " "
			<< node.lon << " " << node.elev << " " << node.lvl << "\n";
	}
	...
	Edge text_readEdge<Edge>(std::istream& is, EdgeID edge_id)
	{
        ...
        WayInfo text_readWay(std::istream& is)
        {
		return readLine(is, [] (std::istream& is) {
                        WayInfo wayInfo;
                        int i_undirected;
			is >> i_undirected >>wayInfo.waylength;
                        if (i_undirected==0) {
                            wayInfo.undirected =false;
                        } else {
                            wayInfo.undirected =true;
                            assert(i_undirected==1);
                        }                        
			return wayInfo;
		});
	}
	...
	namespace FormatFMI_DIST {
        ...
        namespace FormatFMI_DIST_WAY {
		auto Reader_impl::readWayCount() -> uint
		{
			return text_readWayCount(is);
		}
	}
        
        namespace FormatFMI_DIST_WAY {
		auto Reader_impl::readWay() -> WayInfo
		{
			return text_readWay(is);
		}
	}        
