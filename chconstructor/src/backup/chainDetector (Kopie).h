...
public:
    ...
    void collectChain(const chc::NodeID node_id, Chains_and_Remainder &CaR) {
        ...
        //backward direction
        chc::NodeID next = nextChainElement(node_id, type, chc::EdgeType::IN);
        while (next != chc::c::NO_NID) {
            debug_assert(!marked[next]);
            chain.emplace_front(next);
            marked[next] = true;
            chc:: NodeID current = next;
            next = nextChainElement(current, type, chc::EdgeType::IN);            
        }
        
        //backward direction
        next = nextChainElement(node_id, type, chc::EdgeType::OUT);
        while (next != chc::c::NO_NID) {
            debug_assert(!marked[next]);
            chain.emplace_back(next);
            marked[next] = true;
            chc:: NodeID current = next;
            next = nextChainElement(current, type, chc::EdgeType::OUT);            
        }
        
        CaR.addChainOfType(chain, type);
    }
    
    chc::NodeID nextChainElement(const chc::NodeID current, const chc::StreetType streetType, const chc::EdgeType edgeDirection) {
