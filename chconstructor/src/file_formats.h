...
namespace chc {
	...
	template<> void text_writeNode<OSMNode>(std::ostream& os, OSMNode const& node);
        template<> void text_writeNode<WayOSMNode>(std::ostream& os, WayOSMNode const& node);
	...
	template<> void text_writeNode<CHNode<OSMNode>>(std::ostream& os, CHNode<OSMNode> const& node);
        template<> void text_writeNode<CHNode<WayOSMNode>>(std::ostream& os, CHNode<WayOSMNode> const& node);
	...
	template<> Edge text_readEdge<Edge>(std::istream& is, EdgeID edge_id);
        uint text_readWayCount(std::istream& is);
	...
	namespace FormatFMI_DIST
	{
        ...
        namespace FormatFMI_DIST_WAY
	{
		typedef WayOSMNode node_type;
		typedef OSMDistEdge edge_type;
		...
		typedef WayReader<Reader_impl> Reader;
	}
	...
	namespace FormatFMI_CH
	{
        ...
        namespace FormatFMI_CH_WAY
	{
		typedef CHNode<WayOSMNode> node_type;
		typedef CHEdge<OSMEdge> edge_type;
	...
	enum class FileFormat { STD, SIMPLE, FMI, FMI_DIST, FMI_DIST_WAY, FMI_EUCL, FMI_CH, FMI_CH_WAY, FMI_EUCL_CH, STEFAN_CH };
	...
	{
                //return FormatFMI_DIST_WAY::Reader::readGraph(filename);
                //return FormatFMI_DIST_WAY::Reader::readGraph<Node, Edge>(filename);
	...
	{
                //callable(FormatFMI_DIST_WAY::Reader::readGraph(filename));
	...
	inline GraphInData<typename MakeCHNode<typename Writer::node_type>::base_node_type, MakeCHEdge<typename Writer::edge_type>> readGraphForWriter(FileFormat format, std::string const& filename)
        //inline GraphInData<WayOSMNode, MakeCHEdge<OSMDistEdge> > readGraphForWriter(FileFormat format, std::string const& filename)
	{            
                typename Writer::node_type node;
                typename Writer::edge_type edge;
		return readGraph<typename MakeCHNode<typename Writer::node_type>::base_node_type, MakeCHEdge<typename Writer::edge_type>>(format, filename);
                //return readGraph<typename MakeCHNode<WayOSMNode>::base_node_type, MakeCHEdge< OSMDistEdge>>(format, filename);
                //return readGraph<WayOSMNode, OSMDistEdge>(format, filename);
                //return readGraph(format, filename);
	}
	...
	{       
                //callable(readGraphForWriter<FormatFMI_CH_WAY::Writer>(read_format, filename));
                callable(readGraphForWriter<FormatFMI::Writer>(read_format, filename));
                //callable(readGraphForWriter(read_format, filename));
                return;                                
                /*
		switch (write_format) {
		case FileFormat::STD:
	...
	{            
                //writeCHGraphFile<FormatFMI_CH_WAY::Writer>(filename, data);
	...
	{
                writeGraphFile<FormatFMI_CH_WAY::Writer>(filename, data);
		return;
                /*
		switch (format) {
		case FileFormat::STD:
