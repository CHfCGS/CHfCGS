...
class Dijkstra
{
	...
	public:
		Dijkstra(CHGraph<Node, Edge> const& g);
...
template <typename Node, typename Edge>
Dijkstra<Node,Edge>::Dijkstra(CHGraph<Node, Edge> const& g)
...
/*
template <typename Node, typename Edge>
void CHDijkstra<Node,Edge>::_checkForStalling(PQElement const& top)
{
	EdgeType dir(top.direction);
	// TODO When edges are sorted accordingly: loop while
	// edge is up.
	for (auto const& edge: _g._nodeEdges(top.node, dir)) { //changed to get all nodeEdges
		if (_g.isUp(edge, dir)) {
